...
public class ConsultasProdutoMySQL {
    ...
    private static final String SQL_BUSCA_PRODUTO_PRODUCAO = "SELECT * FROM produtos WHERE visivel=1 AND idCategoria=0 ORDER BY nome";
    ...
    public ArrayList<Produto> buscarProdutoProducao() {
        ArrayList<Produto> produtos = new ArrayList<Produto>();
        Connection con;
        PreparedStatement stmt;
        try {
            con = ConexaoMySQL.conectar();
            stmt = con.prepareStatement(SQL_BUSCA_PRODUTO_PRODUCAO);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                Produto prod = new Produto();
                prod.setIdProduto(rs.getInt("codigo_produto"));
